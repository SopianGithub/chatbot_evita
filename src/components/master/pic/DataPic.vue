<template>
  <va-card :title="$t('Master Data Corporate Customer')">
    ...
    <va-button
      @click="tambahCC"
    >
      <va-icon name="fa fa-plus icon-left input-icon"/> Add PIC</va-button>
...
export default {
  ...
  methods: {
    readItems (page = 1, search = null) {
      ...
      axios.get('http://localhost:5000/employe/page', { params })
        .then(response => {
    ...
    edit (pic) {
      this.$router.push({ path: '/admin/master/edit_pic/' + pic.nik })
    ...
    tambahCC () {
      this.$router.push({ path: '/admin/master/add_pic' })
    },
    async deletePIC () {
      await axios.delete(`http://localhost:5000/employe/${this.selectedID}`)
        .then(response => {
